...
exports.isContinuousHTML = isContinuousHTML;
exports.splitContinuousHTML = splitContinuousHTML;
exports.processRenderedLines = processRenderedLines;
/**
 ...
 */
function isWrapperElement(node) {
...
function unwrapElements(nodes) {
    ...
    for (const node of nodes) {
        if (isWrapperElement(node)) {
            ...
            result.push(Object.assign(Object.assign({}, node), { children: unwrapElements(node.children) }));
        }
        else {
            result.push(node);
        }
 ...
 */
function splitNodesByNewlines(nodes, tagContext = []) {
    const lines = [];
    let currentLine = "";
    function flushLine() {
    ...
    function processNodes(nodeList, context) {
 ...
 */
function isContinuousHTML(html) {
 ...
 * ```
 */
function splitContinuousHTML(html) {
    if (!html) {
        return [];
    }
 ...
 * @returns Array of HTML strings, one per line
 */
function processRenderedLines(renderedLines) {
    if (!renderedLines) {
        return [];
    }
