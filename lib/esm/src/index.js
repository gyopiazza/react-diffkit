...
import { mergeHTMLWithDiff, } from "./merge-highlighted-html.js";
...
class DiffViewer extends React.Component {
     ...
     */
    renderWordDiff = (diffArray, renderedHTML) => {
        ...
        if (renderedHTML) {
            ...
            const mergedElements = mergeHTMLWithDiff(renderedHTML, changes, cssClasses);
     ...
     */
    renderLine = (lineNumber, type, prefix, value, additionalLineNumber, additionalPrefix, renderedHTML) => {
        ...
        if (hasWordDiff) {
            ...
            content = _jsx("span", { dangerouslySetInnerHTML: { __html: renderedHTML } });
        }
        else {
    ...
    renderSplitView = ({ left, right }, index) => {
        return (_jsxs("tr", { className: cn(this.styles.line, this.props.lineClassNames?.({ left, right })), children: [this.renderLine(left.lineNumber, left.type, LineNumberPrefix.LEFT, left.value, undefined, undefined, left.renderedHTML), this.renderLine(right.lineNumber, right.type, LineNumberPrefix.RIGHT, right.value, undefined, undefined, right.renderedHTML)] }, index));
    };
    /**
    ...
    renderInlineView = ({ left, right }, index) => {
        ...
        if (left.type === DiffType.REMOVED && right.type === DiffType.ADDED) {
            return (_jsxs(React.Fragment, { children: [_jsx("tr", { className: cn(this.styles.line, this.props.lineClassNames?.({ left })), children: this.renderLine(left.lineNumber, left.type, LineNumberPrefix.LEFT, left.value, null, undefined, left.renderedHTML) }), _jsx("tr", { className: cn(this.styles.line, this.props.lineClassNames?.({ right })), children: this.renderLine(null, right.type, LineNumberPrefix.RIGHT, right.value, right.lineNumber, LineNumberPrefix.RIGHT, right.renderedHTML) })] }, index));
        }
        ...
        return (_jsx("tr", { className: cn(this.styles.line, this.props.lineClassNames?.({ left, right })), children: content }, index));
    };
    /**
    ...
    renderDiff = () => {
        ...
        const { lineInformation, diffLines } = computeLineInformation(oldValue, newValue, disableWordDiff, compareMethod, linesOffset, this.props.alwaysShowLines, oldRenderedLines, newRenderedLines);
