 ...
 */
export function isContinuousHTML(html) {
    ...
    // Check if first line after split is unclosed
    ...
    const openTags = (firstLine.match(/<[^/][^>]*>/g) || []).length;
    const closeTags = (firstLine.match(/<\/[^>]+>/g) || []).length;
    // If there are more opening tags than closing tags, likely continuous
 ...
 * ```
 */
export function splitContinuousHTML(html) {
    if (!html) {
        return [];
    }
 ...
 * @returns Array of HTML strings, one per line
 */
export function processRenderedLines(renderedLines) {
    if (!renderedLines) {
        return [];
    }
